...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    /**
     * Returns a block where the `x` coordinates of `b1`  `b2` are
     ...
     */
    def legalNeighbors: List[(Block, Move)] = neighbors.filter { case (block, move) => terrain(block.b1) && terrain(block.b2) }
